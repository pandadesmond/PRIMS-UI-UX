<template>
    <div class="row q-pa-md">
        ...
        <q-form ref="save_form" class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 bg-white q-my-sm">
            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 q-px-md" style="font-size: 10px;">
                <q-card v-if="!hasFinished">
                    <q-card-section class="row q-px-md">
                        <div v-for="index of 20" :key="index" class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm">
                            <q-skeleton type="text" square style="height:60px;" />
                        </div>
                    </q-card-section>
                </q-card>

                <q-tab-panels v-if="hasFinished" v-model="currentTab" :animated="false" keep-alive ref="tabs">
                    ...
                    <template v-for="tab in ttaTab" :key="tab">
                        <q-tab-panel :name="tab.tab_guid" style="overflow: hidden">
                            ...
                            <div v-if="tab.component_type == 'tir'">
                                <div class="row">
                                    <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                            ...
                            <div v-else>
                                <template v-for="article in tab.tab_article" :key="article">
                                    ...
                                    <div v-else-if="tab.component_type == 'months_rate_percentage'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        <!-- months/rate/% -->
                                    ...
                                    <div v-else-if="tab.component_type == 'target_ratetype_rate_remark'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                    ...
                                    <div v-else-if="tab.component_type == 'days_rate_percentage'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        <!-- days/rate/% -->
                                        <LabelSelectInputRemark1 :label="article.name"
                                            :select1="article.option1"
    ...
    <q-dialog v-model="dialog.distributor" persistent position="top">
        <q-card style="width: 95vw; max-width: 950px; max-height: 90vh; margin-top: 5%;">
                ...
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12" style="font-size: 10px;">
                    <q-tab-panels v-model="dialog.tab" :animated="false" keep-alive ref="dialogTabs" class="q-pa-md">
                        <q-tab-panel name="distributor_assigned" style="overflow: hidden">
                            ...
                            <div v-if="json.approved == 1 && json.rejected == 0">
            ...
            <q-card-actions v-show="dialog.tab == 'distributor_assigned' && (json.approved == 1 && json.rejected == 0)" class="dialog_action" align="right">
...
export default {
    ...
    computed: {
        ...
        tabOptions(){
            if(this.options.tab_list.length<=0)
            {
                this.getTabList();
            }
        ...
        articleOptions(){
            if(this.options.tabArticle_list.length<=0)
            {
                this.getArticleList();
            }
    ...
    async mounted(){
        ...
        if(this.$route.name == 'editTTA')
        {
            ...
            {
                var message = 'Fail to retrieve TTA details.';
                const valid = this.isValidJSON(tta_details.response);
                if(valid)
                {
                    const response = JSON.parse(tta_details.response);
                ...
                {
                    var message = tta_details.response;
                }   
            ...
            if(this.json.terminated == 1 || this.json.renewed == 1 || (this.json.approved == 1 && this.json.rejected == 0))
            {
                ...
                if(tta_tab.status)
                {
                    ...
                    this.ttaTab.map(async curTab =>{
                        ...
                        if(curTab.component_type == "tir")
                        {
                            ...
                            if(tier.status)
                            {
                                curTab.tab_article = tier.response.data.results;
                                if(curTab.tab_article.length>0)
                                {
                        ...
                        else
                        {
                            ...
                            var tab_article_list = curTab.articles;
                            tab_article_list.sort((a,b)=> a.tab_article_info.number - b.tab_article_info.number);
                            tab_article_list.map((curArticle)=>{
                                ...
                                {
                                    article.option2_list = [this.options.calcType_list[0]];
                                }
    ...
    methods:{
        ...
        handleChangeInputNumber(data)
        {
            ...
            else if(data.col.name == 'rate')
            {
                if(Number(data.row.rate) > 100)
                {
                    this.showNotify('negative', 'Percentage cannot more than 100.');
                    data.row.rate = "";
                    return;
        ...
        addField(){
            ...
            if((this.dialog.currentTab.component_type == "dollar_rate_remark" && article.label1 == null) || (this.dialog.currentTab.component_type == "target_ratetype_rate_remark" && (article.label1 == null || article.label2 == null)))
            {
                this.showNotify('negative','The tab type has changed. Please re-add the tab for getting the new component.');
                return;
            }
            ...
            for(var i in this.ttaTab)
            {
                if(this.ttaTab[i].tab_guid == this.dialog.currentTab.tab_guid)
                {
                    ...
                    if(this.dialog.currentTab.component_type == "rate_purgrossnet_date" || this.dialog.currentTab.component_type == "rate_purgrossnet")
                    {
                        if(this.dialog.currentTab.component_type == "rate_purgrossnet_date")
                        {
                            article.date_range_from = this.json.effective_date_from;
                            article.date_range_to = this.json.effective_date_to;
                        }
                    ...
                    else if(this.dialog.currentTab.component_type == "rate_cogsinvnet_date" || this.dialog.currentTab.component_type == "rate_cogsinvnet")
                    {
                        if(this.dialog.currentTab.component_type == "rate_cogsinvnet_date")
                        {
                            article.date_range_from = this.json.effective_date_from;
                            article.date_range_to = this.json.effective_date_to;
                        }
                    ...
                    else if(this.dialog.currentTab.component_type == "dollar_rate_remark")
                    {
                        article.option1_list = [this.options.calcType_list[1]];
                    ...
                    else if(this.dialog.currentTab.component_type == "promotion_percentage")
                    {
                        article.option2_list = [this.options.calcType_list[0]];
                        article.option1 = "month";
                        article.option2 = article.option2_list[0].value;
        ...
        {
            this.dialog.loading = true;
            if(this.json.approved == 1 && this.json.rejected == 0)
            {
                var table_column = [
                    {
                        name: 'action',
        ...
        async table_function_target_incentive_rebate(payload)
        {
            ...
            var table_column = [
                ...
                },
                {
                    ...
                    data_decimal: 2,
                    prefix: this.tir.calc_method == 'purchase_growth_percent' || this.tir.calc_method == 'cogs_growth_percent' || this.tir.calc_method == 'cogs_sales_growth_percent' ? '%' : '$',
                    format_child: '',
                    tooltip: '',
                    headerStyle: 'text-align: center; width: 1%;',
                    filter_type: 'input',
        ...
        async saveTab(tta_guid)
        {
            try{
                    ...
                    if(!new_tab.map(entry => entry.tab_guid).includes(entry.tab_guid)){
                        ...
                        else
                        {
                            if(entry.tab_article.length > 0)
                            {
                                ...
                                entry.tab_article.map(article => {
                                    const obj = {
                                        'tta_tab_article_guid': article.tta_tab_article_guid
                                    }
                    ...
                    if(!original_tab.map(entry=>entry.tab_guid).includes(entry.tab_guid)){
                        ...
                        else
                        {
                            if(entry.tab_article && entry.tab_article.length > 0)
                            {
                                ...
                                entry.tab_article.map(article => {
                                    const obj = {
                                        "date_range_from": article.date_range_from == "" ? null : article.date_range_from,
                                        "date_range_to": article.date_range_to == "" ? null : article.date_range_to,
                                        "option1": article.option1,
                                        "option2": article.option2,
                                        "value1": article.value1,
                                        ...
                                        "created_by": this.username,
                                        "updated_by": this.username,
                                        "tab_article_guid": article.tab_article_guid,
                ...
                for(const new_entry of new_tab)
                {
                    ...
                    if(entry)
                    {
                        ...
                        var payload = {
                            ...
                            pass_json: {
                                ...
                                "tab_guid": new_entry.tab_guid ? new_entry.tab_guid : entry.tab_guid,
                                "tta_guid": tta_guid,
                            }
                        }

                        var pass_obj = {
                            "dispatch": 'tta/trigger_update_tta_tab',
                            "getter": 'tta/get_tab',
                            "app": this,
                            "payload": payload,
                        };
                        ...
                        if(new_entry.component_type=='tir')
                        {
                        ...
                        else
                        {
                            ...
                            new_article.map(new_entry => {
                                ...
                                if(entry){
                                    ...
                                    else if(entry.tta_tab_article_guid)
                                    {
                                        var obj = {
                                            "date_range_from": new_entry.date_range_from == "" ? null : new_entry.date_range_from,
                                            "date_range_to": new_entry.date_range_to == "" ? null : new_entry.date_range_to,
                                            ...
                                            "updated_by": this.username,
                                            "tab_article_guid": new_entry.tab_article_guid,
                                            "tta_tab_guid": entry.tta_tab_guid ? entry.tta_tab_guid : new_entry.tta_tab_guid,
                                            "tta_guid": tta_guid,
                                        }
                                    }
                                    payload.pass_json.push(obj);
                            ...
                            {
                                var pass_obj = {
                                    "dispatch": 'tta/trigger_update_tta_tab_article_bulk',
                                    "getter": 'tta/get_article',
                                    "app": this,
                                    "payload": payload,
                                };
        ...
        async saveTTA(){
            ...
            var payload = {
                pass_json: {
                    ...
                    "settlement_discount": this.json.settlement_discount,
            ...
            if(this.page_function == 'create')
            {
                ...
                if(create_response.status)
                {
                    ...
                    try
                    {
                        ...
                        this.json.distributor_list.map(entry => {
                            const obj = {
                                "created_by": this.username,
                                "updated_by": this.username,
                        ...
                        { 
                            var pass_obj = {
                                "dispatch": 'tta/trigger_create_tta_distributor_bulk',
                                "getter": 'tta/get_distributor',
                                "app": this,
                                "payload": payload,
                            }
                    ...
                    try
                    {
                        const type_guid = `${this.json.division_group}_guid`;

                        var payload = {
                            pass_json: [],
                        }

                        this.json.division.map(guid => {
                            var obj = {
                                "created_by": this.username,
                                "updated_by": this.username,
                        ...
                        {
                            var pass_obj = {
                                "dispatch": 'tta/trigger_create_tta_division_bulk',
                                "getter": 'tta/get_division',
                                "app": this,
                                "payload": payload,
                            }
                    ...
                    try
                    {
                        ...
                        for(var banner of new_banner_obj)
                        {
                            const obj = {
                                "created_by": this.username,
                                "updated_by": this.username,
                                "tta_guid": tta.tta_guid,
                                "concept_guid": banner.concept_guid,
                                "company_info_guid": banner.company_info_guid,
                                "company_guid": this.company_guid,
                            }
                            payload.pass_json.push(obj);
                        }
                        ...
                        {
                            var pass_obj = {
                                "dispatch": 'tta/trigger_create_tta_banner_bulk',
                                "getter": 'tta/get_banner',
                                "app": this,
                                "payload": payload,
                            }
            ...
            else if(this.page_function == 'edit')
            {
                ...
                // for resubmit tta that has been rejected for revision.
                if(this.json.rejected == 1)
                {
                    payload.pass_json.rejected = 0;
                }
                ...
                if(update_response.status)
                {
                    ...
                    try {
                        ...
                        {
                            var pass_obj = {
                                "dispatch": 'tta/trigger_delete_tta_distributor_bulk',
                                "getter": 'tta/get_distributor',
                                "app": this,
                                "payload": payload,
                            };
                        ...
                        {
                            var pass_obj = {
                                "dispatch": 'tta/trigger_update_tta_distributor_bulk',
                                "getter": 'tta/get_distributor',
                                "app": this,
                                "payload": payload,
                            }
                    ...
                    try {
                        ...
                        deleteTask.map(division => {
                            const obj = {
                                'tta_group_guid': division.tta_group_guid,
                            }
                            payload.pass_json.push(obj);
                        })
                        console.log("payload",payload);
                        ...
                        {
                            var pass_obj = {
                                "dispatch": 'tta/trigger_delete_tta_division_bulk',
                                "getter": 'tta/get_division',
                                "app": this,
                                "payload": payload,
                            };
                    ...
                    try {
                        ...
                        deleteTask.map(banner => {
                            const obj = {
                                "tta_concept_guid": banner.tta_concept_guid,
                            }
                            payload.pass_json.push(obj);
                        })
                        console.log("payload",payload);
                        ...
                        {
                            var pass_obj = {
                                "dispatch": 'tta/trigger_delete_tta_banner_bulk',
                                "getter": 'tta/get_banner',
                                "app": this,
                                "payload": payload,
                            };
                ...
                else
                {
                    ...
                    {
                        var message = update_response.response;
                    }   
                    this.showNotify('negative', message);
                }
            }
        ...
        async validateTTA()
        {
            ...
            // validate TIR table
            if(this.ttaTab.map(entry=>entry.component_type).includes('tir'))
            {
                ...
                for(var i = 0; i < tier.length; i++)
                {
                    if(i == (tier.length-1))
                    {
                        if(tier[i].rate == "")
                        {
                            this.showNotify('negative', 'Please fill in rate for the last tier.');
                            this.showLoading = false;
                            return;
                        }
                    }
                    ...
                    {
                        if(tier[i].rate == "" || tier[i].rebate_to == "")
                        {
        ...
        async isExistTTA(guid)
        {
            ...
            if(data_response.status)
            {
                tta_list = data_response.response.data.results.filter(entry=>entry.refno != this.json.refno);
            }
        ...
        async handleAuthorise()
        {
            ...
            if(!this.json.supplier_to || this.json.supplier_to == "")
            {
