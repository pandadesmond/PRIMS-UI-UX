<!-- TTA that display tab_article based on tab_article type (hardcoded) -->
<template>
                ...
                <q-btn-group class="btn_grp2" style="margin-right: 10px;">
                    <q-btn-dropdown icon="build" rounded color="primary" split size="9px" no-caps>
                        <q-list padding style="" class="btn_qlist">
                            <q-item v-if="json.authorised == 0 && json.rejected == 0 && json.terminated == 0" clickable style="padding: 0px">
                                <q-item-section>
                                    <q-btn :disable="false" flat no-caps :label="'Authorise'" color="primary" @click="dialog.authorise = true" class="btn_drp"/>
                                </q-item-section>
                            </q-item>
                            
                            <q-item v-if="json.authorised == 1 && json.approved == 0 && json.rejected == 0 && json.terminated == 0" clickable style="padding: 0px">
                                <q-item-section>
                            ...
                            <q-item v-if="json.authorised == 1 && json.approved == 1 && json.rejected == 0 && json.terminated == 0" clickable style="padding: 0px">
                                <q-item-section>
                    ...
                    <q-tabs v-model="currentTab" active-class="active_class_tab" indicator-color="transparent" align="justify" inline-label dense narrow-indicator shrink stretch no-caps>
                        ...
                        <template v-for="tab in ttaTab" :key="tab">
                            <q-tab :name="tab.type" :label="tab.tab_info ? tab.tab_info.name : tab.name">
                                <q-btn icon="close" class="q-ml-xs" size="12px" @click="removeTab(tab)" flat dense/>
                            </q-tab>
                        </template>
                ...
                <q-tab-panels v-model="currentTab" :animated="false" keep-alive ref="tabs">
                    <q-tab-panel name="supplier_profile" style="overflow: hidden">
                        <div class="row">
                            ...
                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelMultiselect label="Distributor (Data Source)" v-model:pass_value="json.distributor" :options="options.distributor_list" :readonly="readonlyStatus" option_label="Distributors"
                                :forceSelectAll="forceSelectAllDistributor" :dbComponentBehavior="dbComponentBehavior.text" :manage_button="true" @receiveClickManage="handleDistributorDialog"/>
                            </div>

                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelMultiselect label="Division" v-model:pass_value="json.division" :options="options.division_list" :readonly="readonlyStatus" option_label="Divisions"
                                :forceSelectAll="forceSelectAllDivision" :dbComponentBehavior="dbComponentBehavior.text"/>
                            </div>

                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelDatepicker label="Effective Date From" :daterange="json.effective_date_from" :optionsFn="optionsDateEndNew" v-on:receiveChange="handleChangeStartDate" 
                                :dbComponentBehavior="dbComponentBehavior.text_required" :readonly="readonlyStatus"/>
                            </div>

                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelDatepicker label="Effective Date To" :daterange="json.effective_date_to" :optionsFn="optionsDateEndNew" v-on:receiveChange="handleChangeEndDate" 
                                :dbComponentBehavior="dbComponentBehavior.text_required" :readonly="readonlyStatus"/>
                            </div>

                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelSelect label="Banner" v-model:pass_value="json.concept_guid" :options="options.banner_list" :readonly="readonlyStatus" :dbComponentBehavior="dbComponentBehavior.select"/>
                            </div>
                            ...
                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelInput label="Supplier PIC" v-model:pass_value="json.supplier_pic_name" :readonly="readonlyStatus" :dbComponentBehavior="dbComponentBehavior.text"/>
                            </div>

                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelInput label="Designation" v-model:pass_value="json.supplier_pic_designation" :readonly="readonlyStatus" :dbComponentBehavior="dbComponentBehavior.text"/>
                            </div>

                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                <LabelInput label="Contact" v-model:pass_value="json.supplier_pic_contact" :readonly="readonlyStatus" :dbComponentBehavior="dbComponentBehavior.text"/>
                            </div>
                        ...
                        <q-tab-panel :name="tab.type" style="overflow: hidden">
                            ...
                            <div v-if="tab.type == 'TIR'">
                                <div class="row">
                                    ...
                                    <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        <LabelDaterange label="Cut Off Date" :daterange="tir.cutoff_date" :optionsFn="optionsDateCutOff" v-on:receiveChange="handleChangeCutOffDate" 
                                        :dbComponentBehavior="dbComponentBehavior.text" :readonly="readonlyStatus"/>
                                    </div>
                                </div>
                            ...
                            <div v-else>
                                <template v-for="article in tab.tab_article" :key="article">
                                    ...
                                    <div v-if="article.form_type == 'ratetype/rate/pur_grossnet/daterange'" 
                                    ...
                                    <div v-else-if="article.form_type == 'ratetype/rate/pur_grossnet'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                    ...
                                    <div v-else-if="article.form_type == '$/rate/remark'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        <!-- $/rate/remark -->
                                        <LabelSelectInputRadio :label="article.name"
                                            v-model:select="article.option1"
                                    ...
                                    <div v-else-if="article.form_type == 'months/rate/%'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        <!-- months/rate/% -->
                                        <LabelSelectInputRemark1 :label="article.name"
                                            ...
                                            :radioOptions="[{label:'Order Within X Months',value:'OrderWithinXMonths'}]"
                                    ...
                                    <div v-else-if="article.form_type == 'ratetype/rate/inv_grossnet/daterange'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        ...
                                        <LabelSelectInputRadioDate :label="article.name"
                                            ...
                                            v-model:date_from="article.date_range_from"
                                            v-model:date_to="article.date_range_to"
                                            :selectOptions="options.option1_list"
                                            :radioOptions="options.remark1_list"
                                            :allow_remove="readonlyStatus ? false : true"
                                            :readonly="readonlyStatus"
                                            @receiveRemove="removeField(article)"
                                        />
                                    ...
                                    <div v-else-if="article.form_type == 'ratetype/rate/inv_grossnet'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                    ...
                                    <div v-else-if="article.form_type == 'target/target_ratetype/ratetype/rate/remark'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        <!-- target/target_ratetype/ratetype/rate/remark -->
                                        <LabelSelectInputRemark2 :label="article.name"
                                            :select1="json.option1"
                                            :radio1="json.remark1"
                                            :input1="json.value1"
                                            :selectOptions1="options.option1_list"
                                            :radioOptions1="article.remark1_list"
                                            :select2="json.option2"
                                            :radio2="json.remark2"
                                            :input2="json.value2"
                                            :selectOptions2="options.option2_list"
                                    ...
                                    <div v-else-if="article.form_type == 'days/rate/%'" 
                                        class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-xs-12 q-px-sm q-py-xs">
                                        <!-- days/rate/% -->
                                        <LabelSelectInputRemark1 :label="article.name"
                                            :select1="article.option1"
                                            ...
                                            :selectOptions2="options.option2_list"
                                            :radio="article.remark1"
                                            :radioOptions="[{label:'Order Within X Days',value:'OrderWithinXDays'}]"
                                            @receiveChange="newVal => handleChangeDayMonthRate(article,newVal)"
                                            :allow_remove="readonlyStatus ? false : true"
                                            :readonly="readonlyStatus"
                                            @receiveRemove="removeField(article)"
    ...
    <q-dialog v-model="dialog.distributor" persistent position="top">
        <q-card style="width: 95vw; max-width: 950px; max-height: 90vh; margin-top: 5%;">
            ...
            <q-card-actions v-show="dialog.tab == 'distributor_assigned'" class="dialog_action" align="right">
    ...
    <q-dialog v-model="dialog.add_field" persistent position="top">
        <q-card style="width: 95vw; max-width: 600px; max-height: 90vh; margin-top: 5%;">
            ...
            <q-card-section class="row items-center dialog_header_custom">
                ...
                <q-space />
                <q-btn icon="close" flat round dense :disable="dialog.loading" @click="dialog.add_field = false" />
            </q-card-section>
            ...
            <q-card-actions class="dialog_action" align="right">
                <Button v-on:receiveClick="dialog.add_field = false"
                    :pass_label="'Cancel'"
                    :pass_no_caps="true"
                    :pass_square="true"
                    :pass_dense="true"
...
export default {
    ...
    data() {
        return {
            ...
            options: {
                ...
                calcMethod_list: [{value: 'gpv', label: 'Gross Purchase Value'},{value: 'npv', label: 'Net Purchase Value'},{value: 'consignment', label: 'Cost of Goods Sold (Consignment)'},{value: 'growth', label: 'Growth %'}],
            },
            table_data:{
                distributor: [],
                audit_trail: [],
    ...
    async mounted(){
        ...
        if(division_list.status)
        {
            ...
            list.forEach(element => {
                ...
                element.label = `${element.group_desc} | ${element.dept_desc} | ${element.subdeptdesc} | ${element.category_desc}`;
            });
            this.options.division_list = list;
        }
        ...
        if(this.$route.name == 'editTTA')
        {
            ...
            if(tta_details.status)
            {
                ...
                if(this.json.rejected == 1 || this.json.terminated == 1 || this.json.renewed == 1)
                {
                ...
                if(division.status)
                {
                    ...
                    if( data.count > 0)
                    {
                        this.json.division = data.results.map((entry)=>{return entry.dept_guid});
                    ...
                    for(var i in this.ttaTab)
                    {
                        ...
                        if(this.ttaTab[i].type == "TIR")
                        {
                            ...
                            if(tier.status)
                            {
                                ...
                                this.tir.cutoff_date = {
                                    from: this.ttaTab[i].tab_article[0].cutoff_date_from,
                                    to: this.ttaTab[i].tab_article[0].cutoff_date_to,
                                };
                            }
                        ...
                        else
                        {
                            ...
                            if(tab_article.status)
                            {
                                ...
                                for(var x in tab_article_list)
                                {
                                    ...
                                    if(article.type == 'RB01' || article.type == 'RB02' || article.type == 'RB03' || article.type == 'RB04' 
                                    || article.type == 'ALW01' || article.type == 'ALW02' || article.type == 'ALW03' || article.type == 'ALW04' || article.type == 'ALW05' || article.type == 'ALW06')
                                    {
                                        // ratetype/rate/pur_grossnet/daterange
                                        if(article.type == 'RB01' || article.type == 'RB02' || article.type == 'RB03' || article.type == 'RB04')
                                        {
                                            article.form_type = "ratetype/rate/pur_grossnet/daterange";
                                        }
                                        // ratetype/rate/pur_grossnet
                                        else if(article.type == 'ALW01' || article.type == 'ALW02' || article.type == 'ALW03' || article.type == 'ALW04' || article.type == 'ALW05' || article.type == 'ALW06')
                                        {
                                            article.form_type = "ratetype/rate/pur_grossnet";
                                        }
                                        article.option1_list = this.options.calcType_list;
                                        article.remark1_list = this.options.pur_grossnet_list;
                                    }
                                    else if(article.type == 'NST01' || article.type == 'NST02' || article.type == 'NST03' || article.type == 'NST04' || article.type == 'NST05' || article.type == 'NST06'
                                    || article.type == 'STRE01' || article.type == 'STRE02' || article.type == 'STRE03' || article.type == 'STRE04' || article.type == 'STRE05'
                                    || article.type == 'EPS01' || article.type == 'EPS02' || article.type == 'EPS03'
                                    || article.type == 'AF01' || article.type == 'AF02' || article.type == 'AF03' || article.type == 'AF04'
                                    || article.type == 'PEN01' || article.type == 'PEN02' || article.type == 'PEN03' || article.type == 'PEN04' || article.type == 'PEN05' || article.type == 'PEN06')
                                    {
                                        // $/rate/remark
                                        article.form_type = "$/rate/remark";
                                        article.option1_list = [this.options.calcType_list[1]];
                                        article.remark1_list = [{value:'PerStorePerSKU', label:'Per Store Per SKU'}];
                                    }
                                    else if(article.type == 'PDFE01' || article.type == 'PDFE02' || article.type == 'PDFE03' || article.type == 'PDFE04')
                                    {
                                        // months/rate/%
                                        article.form_type = "months/rate/%";
    ...
    methods:{
        ...
        handleChangeCutOffDate(newVal)
        {
            ...
            this.ttaTab.map((entry)=>{
                if(entry.type == "TIR")
                {
                    for(var i in entry.tab_article)
                    {
                        entry.tab_article[i].cutoff_date_from = newVal.from;
                        entry.tab_article[i].cutoff_date_to = newVal.to;
                    }
                }
            });
        },
        ...
        handleChangeCalcMethod(newVal)
        {
            this.ttaTab.map((entry)=>{
                if(entry.type == "TIR")
                {
                    for(var i in entry.tab_article)
                    {
                        entry.tab_article[i].calc_method = newVal;
                    }
                }
            });
            this.table_function_target_incentive_rebate(this.ori_params.target_incentive_rebate);
        ...
        addField(){
            ...
            || article.type == 'ALW01' || article.type == 'ALW02' || article.type == 'ALW03' || article.type == 'ALW04' || article.type == 'ALW05' || article.type == 'ALW06')
            {
                ...
                {
                    this.showNotify('negative','Please select TTA effective date from and effective date to.');
                    return;
                }
            }
        ...
        addTier()
        {
            ...
            if(this.tir.cutoff_date.from == "" || this.tir.cutoff_date.to == "")
            {
                this.showNotify('negative','Please select cut off date.');
                return;
            }
            ...
            const tier = {
                ...
                "cutoff_date_from": this.tir.cutoff_date.from,
                "cutoff_date_to": this.tir.cutoff_date.to,
        ...
        async saveTab(tta_guid)
        {
            try{
                ...
                const deleteTask = original_tab.filter(async (entry)=>{
                ...
                const editTask = new_tab.map(async (new_entry)=>{
                    ...
                    if(entry){
                        ...
                        else
                        {
                            ...
                            original_article.filter(async (entry)=>{
                            ...
                            new_article.filter(async (entry)=>{
                            ...
                            new_article.filter(async (new_entry)=>{
        ...
        async saveTTA(){
            ...
            if(this.page_function == 'create')
            {
                ...
                if(create_response.status)
                {
                    ...
                    try
                    {
                        for(var guid of this.json.distributor)
                        {
                            var payload = {
                                pass_json: {
                                    "created_by": this.username,
            ...
            else if(this.page_function == 'edit')
            {
                ...
                if(update_response.status)
                {
                    ...
                    try {
                        ...
                        var new_distributor = this.json.distributor;

                        // Delete tta distributor from original list if it does not include in new list
                        original_distributor.filter(async (entry)=>{
                            if(!new_distributor.includes(entry.supplier_guid)){
                        ...
                        new_distributor.filter(async (entry)=>{
                            if(!original_distributor.map(entry=>entry.supplier_guid).includes(entry)){
                    ...
                    try {
                        ...
                        original_division.filter(async (entry)=>{
                            if(!new_division.includes(entry.dept_guid)){
                                console.log('delete division',entry)

                                var payload = {
                                    'tta_division_guid': entry.tta_division_guid,
                                }
                                var pass_obj = {
                        ...
                        new_division.filter(async (entry)=>{
        ...
        handlePrint()
        {
            this.$router.push({name: 'printTTA', query: {tta_guid: this.currentTTA}});
        },
        goBack(){
            this.$router.push({name: 'tta'});
        },
