<template>
    ...
    <q-dialog v-model="delete_dialog" persistent position="top">
        ...
        <q-card-actions align="right" style="padding-top:0px;padding-bottom:16px;padding-right:16px">
            ...
            <q-btn flat label="Confirm" font_color="white" color="'#D81111'" size="12px" class="dialog_confirm_delete_button" @click="handleDeleteTab" />
        </q-card-actions>
...
export default {
    ...
    methods: {
            ...
            else
            {
                var column = [
                    ...
                    },
                    {
                        ...
                        format:(val)=> this.componentTypeOptions.find(entry=>entry.value == val).label,
                        headerStyle: 'text-align: center; width: 1%;',
                        filter_type: 'input',
                        filter_options: [],
                        filter_value: '',
        ...
        {
            this.$router.push({name: 'editTab', query: { guid: row.row.tab_guid}});
        },
        handleCreate(){
            this.$router.push({name: 'createTab'});
        },
        handleChangeSearch()
        {
        ...
        async handleDeleteTab()
        {
            ...
            var payload = {
                params: {
                    'tab_guid': this.deleteItem.tab_guid,
                }
            }
            ...
            for(const article of data_response.response.data.results)
            {
                var payload = {
                ...
                if(!delete_response.status)
                {
                    ...
                    var message = `Article ${article.name} delete failed.\n`;
                    if(valid)
                    {
                    ...
                    {
                        message = message + JSON.stringify(delete_response.response);
                    }
            ...
            var pass_obj = {
                "dispatch": 'general/trigger_delete_tab',
                "getter": 'general/get_tab',
                "app": this,
                "payload": payload,
            };
