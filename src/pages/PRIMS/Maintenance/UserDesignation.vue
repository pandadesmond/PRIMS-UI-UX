<template>
    ...
    <q-dialog v-model="delete_dialog" persistent position="top">
        ...
        <q-card-actions align="right" style="padding-top:0px;padding-bottom:16px;padding-right:16px">
            ...
            <q-btn flat label="Confirm" font_color="white" color="'#D81111'" size="12px" class="dialog_confirm_delete_button" @click="handleDeleteUserDesignation" />
        </q-card-actions>
...
export default {
    ...
    methods: {
        ...
        {
            this.$router.push({name: 'editUserDesignation', query: { guid: row.row.designation_guid}});
        },
        handleCreate(){
            this.$router.push({name: 'createUserDesignation'});
        },
        handleChangeSearch()
        {
            this.forceLoading = true;
        ...
        async handleDeleteUserDesignation()
        {
            ...
            var pass_obj = {
                "dispatch": 'user/trigger_delete_designation',
                "getter": 'user/get_designation',
                "app": this,
                "payload": payload,
            }
