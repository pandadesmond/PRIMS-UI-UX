  ...
  <div class="card_sides">
    <q-card style="min-height: 100vh;">
      <q-card-section class="header_top_mobile">
        ...
        <Button_icon :disable="showAddLoading" :text="header=='SO' ? 'New SO' : 'New SI'" :big_round="true" :flat="true" :font_color="'#1976D2'" :color="'#f1f8ff'" :icon="'add_circle'" :outline="false" size="15px" class="custom_more_button" @click="handleCreate()" />
      </q-card-section>
      ...
      <q-card-section class="dialog_header card_sides">
        <div class="card_ypadding">
          ...
          <div class="row justify-center">
            <Label v-if="header=='SO' && type=='create'" :text="'Your Sales Order is Ready'"
            style="background-color: white;"/>

            <Label v-if="header=='SO' && type=='update'" :text="'Sales Order Updated'"
            style="background-color: white;"/>

            <Label v-if="header=='SI' && type =='create'" :text="'Your Sales Invoice is Ready'"
            style="background-color: white;"/>

            <Label v-if="header=='SI' && type =='update'" :text="'Sales Invoice Updated'"
            style="background-color: white;"/>
          ...
          <div class="row q-pt-lg" style="font-size: 10px;">
              ...
              <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                  ...
                  <div class="row">
                    ...
                    <div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 col-xl-7" v-if="header=='SO'"><b>:</b> {{ json.PODate }}</div>
                    <div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 col-xl-7" v-if="header=='SI'"><b>:</b> {{ json.GRDate }}</div>
            ...
            <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 col-xl-6">
              <div class="row">
                ...
                <div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 col-xl-7" v-if="header=='SO'"><b>:</b> {{ json.SCode }}</div>
              ...
              <div class="row">
                ...
                <div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 col-xl-7" v-if="header=='SO'"><b>:</b> {{ json.SName }}</div>
...
export default {
  ...
  methods: {
    async getFilterSimain(payload)
    {
      return await this.$store.dispatch('sales/trigger_read_si_by_refno', payload).then(() => {
          var data = JSON.parse(JSON.stringify(this.$store.getters['sales/get_read_si_by_refno']));
          return data;
      });
    },
    async getFilterSomain(payload)
    {
    ...
    async getSochild(payload)
    {
      return await this.$store.dispatch('sales/trigger_get_sochild', payload).then(() => {
          var data = JSON.parse(JSON.stringify(this.$store.getters['sales/get_sochild']));
          return data;
      });
    },
    ...
    back()
    {
      if(this.header == 'SO')
      {
        if(this.type == 'create')
        {
          this.$router.push({ name: 'EditSalesOrder', query: { refno: this.refno, step: 2 } })
        }
        else if(this.type == 'update')
        {
          this.$router.push({ name: 'EditSalesOrder', query: { refno: this.refno } })
        }
      }
      else if(this.header == 'SI')
      {
        // if(this.type == 'create')
        // {
        //   this.$router.push({ name: 'EditSalesInvoice', query: { refno: this.refno, step: 2 } })
        // }
        // else if(this.type == 'update')
        // {
          this.$router.push({ name: 'EditSalesInvoice', query: { refno: this.refno } })
        // }
      }
    },
    handleCreate()
    {
      if(this.header == 'SO')
      {
        this.$router.push({ name: "CreateSalesOrder" });
      }
      else if(this.header == 'SI')
      {
        this.$router.push({ name: "CreateSalesInvoice" });
      }
  ...
  async created(){
    ...
    if(this.header == 'SO')
    {
      ...
      var cdata = await this.getSochild(cpayload);
      ...
      var data = await this.getFilterSomain(payload);
    ...
    else if(this.header == 'SI')
    {
      ...
      var data = await this.getFilterSimain(payload)
    ...
    if(this.header == 'SO')
    {
      cdata = await this.getSochild(cpayload);
    }
    else if(this.header == 'SI')
    {
      cdata = await this.getSichild(cpayload);
    }
    else if(this.header == 'DN')
    {
      var dpayload = {
