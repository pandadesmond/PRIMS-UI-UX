<template>
    ...
    <div class="row row_section" style="padding: 0px; margin-bottom: 10px; margin-left: 12px;">
        ...
        <Chip text="Location" icon="fmd_good" /> /
        <Chip
        :text="path=='currency' ? 'Currency' : 'Continent'"
        :icon="path=='currency' ? 'attach_money' : 'south_america'"
        />
        </div>

        <Title :text="path=='currency' ? 'Currency' : 'Continent'" :noFilter="true" :filter_status="filter_status" v-on:receiveFilter="handleReceiveFilter" />
    </div>
        ...
        <q-card style="width: 700px; max-width: 80vw;">
            ...
            <Label
                :text="is_update ? `Edit ${path}` : `Add ${path}`"
...
export default{
    ...
    methods: {
        ...
        handleDelete(payload){
            if(this.path=='currency')
            {
              this.active_delete_json.guid = payload.cur_guid;
            }
            else if(this.path=='continent')
            {
              this.active_delete_json.guid = payload.con_guid;
            }
        ...
        async handleDeleteLine(){
            ...
            {
              var payload = {
                  cur_guid: this.active_delete_json.guid
              }

              await this.$store.dispatch('location/trigger_delete_mc_currency', payload).then(async () => {
                  data = JSON.parse(JSON.stringify(this.$store.getters['location/get_mc_currency_status']));
              });
            }
            else if(this.path=='continent')
            {
              var payload = {
                  con_guid: this.active_delete_json.guid
              }

              await this.$store.dispatch('location/trigger_delete_mc_continent', payload).then(async () => {
                  data = JSON.parse(JSON.stringify(this.$store.getters['location/get_mc_continent_status']));
              });
            }
        ...
        async handleSave()
        {
            ...
            if(!this.is_update)
            {
                ...
                if(this.path=='currency')
                {
                  await this.$store.dispatch('location/trigger_create_mc_currency', payload).then(() => {
                      data = JSON.parse(JSON.stringify(this.$store.getters['location/get_mc_currency_status']));
                  });
                }
                else if(this.path=='continent')
                {
                  await this.$store.dispatch('location/trigger_create_mc_continent', payload).then(() => {
                      data = JSON.parse(JSON.stringify(this.$store.getters['location/get_mc_continent_status']));
                  });
                }
            }
            else if(this.is_update)
            {
                ...
                {
                  payload = {
                      cur_guid: this.json_new.cur_guid,
                      pass_json: pass_json
                  }

                  await this.$store.dispatch('location/trigger_update_mc_currency', payload).then(() => {
                      data = JSON.parse(JSON.stringify(this.$store.getters['location/get_mc_currency_status']));
                  });
                }
                else if(this.path=='continent')
                {
                  payload = {
                      con_guid: this.json_new.con_guid,
                      pass_json: pass_json
                  }

                  await this.$store.dispatch('location/trigger_update_mc_continent', payload).then(() => {
                      data = JSON.parse(JSON.stringify(this.$store.getters['location/get_mc_continent_status']));
                  });
                }
            }
