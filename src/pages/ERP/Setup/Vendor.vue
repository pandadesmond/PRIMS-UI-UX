                                  ...
                                  <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 q-pb-sm">
                                    <div class="row" style="align-items: center;">
                                      <div class="col-xl-5 col-lg-5 col-md-5 col-sm-3 col-xs-3">
                                          <Label :pass_value="'Supplier Link'" v-on:receiveClick="openDialog_SupplierLink"/>
                                      </div>
                        ...
                        <q-tab-panel name="Configuration" style="padding: 0px; padding-top: 10px; color: grey; overflow: hidden">
                            ...
                            <div class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-bottom: 3px;">
                                    ...
                                    <div class="row col-xl-2 col-lg-2 col-md-2 col-sm-2 col-xs-2">
                                        ...
                                        :readonly="page_function == 'view' ? true : false"
                                            :autofocusclick="true"
                                            v-model:pass_value="json.sc_po_min_amt"
                            ...
                            <div class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-bottom: 3px;">
                                ...
                                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding-right: 10px">
                                    ...
                                    :readonly="page_function == 'view' ? true : false"
                                    :autofocusclick="true"
                                    v-model:pass_value="json.sc_limit_control_amt"
                                    ...
                                    <tbody>
                                      <template v-for="(row, index) in product_table_data.data.results" :key="index">
                                        <tr>
                                          ...
                                          <td style="padding: 10px; padding-top: 5px; padding-bottom: 5px; border: 1px solid #cbcaca;">
                                            <q-toggle
                                            :disable="true"
                                            dense
                                            style="padding-top: 7px; padding-bottom: 7px;"
                                            color="green"
                                            v-model="row.unit.priority_vendor"
                                            :true-value="1"
                                            :false-value="0"
                                            />
                                          </td>
                                          <td style="padding: 10px; padding-top: 5px; padding-bottom: 5px; border: 1px solid #cbcaca;">{{ row.unit.imsc_product_type }}</td>
                                          ...
                                          <td style="padding: 10px; padding-top: 5px; padding-bottom: 5px; border: 1px solid #cbcaca;">{{ row.unit.UM }}</td>
...
<q-dialog v-model="suplink_dialog" persistent position="top">
    <q-card style="width: 95vw; max-width: 1100px; max-height: 90vh; margin-top: 5%;">
      ...
      <q-card-section class="row items-center dialog_header_custom">
          ...
          <q-space />
          <q-btn icon="close" flat round dense :disable="suplink_loading" @click="closeItemDialog" />
      </q-card-section>
          ...
          <div class="row dialog_content">
            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 q-pl-xs">
              <div style="width: 100%;">
                ...
                :row_per_page="[5,10,20,50,1000]"
                :title="table_title"
                :table_data="supLink_table_data"
                :table_column="supLink_table_column"
                :flat_status="true"
                :bordered_status="true"
                v-on:receiveRequestTable="supLinkHandleTableChange"
                :allow_add="false"
                :allow_edit="true"
                :allow_delete="false"
                :allow_select="true"
                ...
                v-on:receiveColumnRearrange="handleColumnRearrange"
                v-on:filter_column="handleFilterColumn"
                :forceLoading="suplinkForceLoading"
                :column_reordering="true"
                :row_reordering="false"
                v-on:receiveHandleClearFilter="handleClearFilter"
                :pass_visible_columns="visibleColumns"
...
data() {
  return{
    supLink_table_column:[],
    supLink_table_data:[],
    suplink_ori_params: {},
    rearrange_column: [],
    suplinkForceLoading: false,
    filter_mode_on: true,
    filter_options: [],
    filter_value: [],
    suplink_dialog: false,
...
async created()
{
    ...
    if(this.$route.name == "VendorMaster" || this.$route.name == "Vendor" || this.$route.name == "VendorView")
    {
        this.supcus_type = 'S';
    }
    else if(this.$route.name == "CustomerMaster" || this.$route.name == "Customer" || this.$route.name == "CustomerView")
    {
        this.supcus_type = 'C';
    }
...
methods:{
  ...
  async addItem()
    {
      // this.suplink_loading = true;

      if(!this.supLink_table_data.data)
      {
        this.showNotify('negative', 'Please select row to continue');
        return;
      }

      var selected_row = JSON.parse(JSON.stringify(this.supLink_table_data.data.results.find(row => row.select === true)));
      if(!selected_row)
      {
        this.showNotify('negative', 'Please select row to continue');
        return;
      ...
      this.json.AccountCode = selected_row.AccountCode;
      this.json.Code = selected_row.Code;
      this.json.Name = selected_row.Name;
      this.json.name_reg = selected_row.name_reg;
      this.suplink_loading = false;
      this.suplink_dialog = false;
      
    },
  handleFilterColumn(pass_payload)
  {
      // this.rearrange_column = this.suplink_table_column;
      this.supplierLink_table_function(this.suplink_ori_params);
  },
  handleSelectItem(row)
    {
      this.$nextTick(()=>{
        // console.log('suplink_data',this.supLink_table_data.data.results)
        for(var i in this.supLink_table_data.data.results)
        {
          var new_row =  this.supLink_table_data.data.results[i]
  ...
  {
      for (const index in this.suplink_table_column) {
          
          var is_array = Array.isArray(this.suplink_table_column[index].filter_value);
          if(is_array)
          {
              this.suplink_table_column[index].filter_value = [];
          }
          else
          {
              this.suplink_table_column[index].filter_value = '';
  ...
  supLinkHandleTableChange: function(newVal)
    {
          ...
          this.suplink_ori_params = payload;
          this.supplierLink_table_function(payload);
    },
  async supplierLink_table_function(ori_payload){
        // console.log('ori_payload',ori_payload)
          this.suplinkForceLoading = true;
          ...
          for (const index in this.supLink_table_column) {
              ...
              if(is_array)
              {
                ...
                for (const value_index in this.supLink_table_column[index].filter_value) {
                    value += this.supLink_table_column[index].filter_value[value_index]+",";
                }

                value = value.replace(/,\s*$/, "");

                payload[this.supLink_table_column[index].field] = value
              }
              else
              {
                if(this.supLink_table_column[index].filter_value)
                {
                    payload[this.supLink_table_column[index].field] = this.supLink_table_column[index].filter_value
                }
                else
                {
                    delete payload[this.supLink_table_column[index].field]
                }
              }
            }
          ...
          this.supLink_table_column = table_column;

          this.suplinkForceLoading = false;
  },
  closeItemDialog()
    {
      this.suplink_dialog = false;
      ...
      this.suplink_table_data = [];
      this.suplink_table_column = [];
    },
  openDialog_SupplierLink()
  {
    if(this.page_function != 'create')
    {
      return;
    }
    this.suplink_dialog= true;
  },
  ...
  handleChangeAllDay(newVal, index)
  {
    ...
    if (trueCount > this.json.stock_day) {
      ...
      this.showNotify('negative', 'The number of selected days exceeds stock day limit.');
    } else {
    ...
    if(dispatch_response.status)
    {
        ...
        if(results.length > 0)
        {
            for(var i in results)
            {
              ...
              if(results[i].imsc_product_type == '0')
              {
                results[i].imsc_product_type = 'NA'
              }
              if(results[i].imsc_product_type == '1')
              {
                results[i].imsc_product_type = 'Local'
              }
              if(results[i].imsc_product_type == '2')
              {
                results[i].imsc_product_type = 'Indent'
              }
              if(results[i].imsc_product_type == '3')
              {
                results[i].imsc_product_type = 'Locally Indent'
              }

              results[i].unit = {
                no: Number(i) + 1,
                priority_vendor: results[i].priority_vendor,
                imsc_product_type: results[i].imsc_product_type,
                UM: results[i].UM,
                ...
                effective_date: results[i].future_EffDate,
                uom2: 'Unit/Case',
                expected_cost: '0',
                disc12: '0',
                disc22: '0',
              ...
              results[i].case = {
                ...
                order_uom: results[i].um_case ? results[i].um_case : 0, 
                uom_case: 'Unit',
                cost: '0',
  ...
  async trigger_search_supcus(payload){
    ...
    if(dispatch_response_supcus_vendor.status){
      ...
      this.json.sc_po_min_amt = filter.sc_po_min_amt;
      this.json.sc_limit_control_amt = filter.sc_limit_control_amt;
  ...
  back(){        
        ...
        {
          this.$router.push("CustomerMaster");
        }
        else
        {
          this.$router.push("VendorMaster");
  ...
  async handleCreateVendor(){
    ...
    async createVendor() {
      ...
      var sc_po_min_amt = this.json.sc_po_min_amt
      var sc_limit_control_amt = this.json.sc_limit_control_amt
      ...
      var payload = {
          pass_json: {
              ...
              sc_po_min_amt : sc_po_min_amt,
              sc_limit_control_amt: sc_limit_control_amt,
