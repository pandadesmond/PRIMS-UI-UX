<template>
  <div v-if="$q.screen.width > 599" class="card_sides card_section">
    <div :class="page_function == 'CreateDebitNote' ? 'chip-container_2' : 'chip-container_1'">
       <div v-if="page_function == 'CreateDebitNote'" class="text-subtitle1 header_text">
         New Debit Note
        </div>
  ...
  <div v-else class="card_sides">
    ...
    <div :class="page_function == 'CreateDebitNote' ? 'chip-container_2' : 'chip-container_1'">
          <Chip v-if="page_function == 'CreateDebitNote'"  :label="$language('D0202')" :icon="'chevron_left'" text-color="grey-10" class="bold-font chip_sty back_chip_icon" @click="back()" />
          <Chip v-if="page_function == 'EditDebitNote'" :icon="'chevron_left'" :square="true" :label="'Edit Surcharge Code' +' / '+json.Itemcode" text-color="grey-10" class="bold-font chip_sty" v-on:receiveClick="handleNavigate('Itemmaster')"/>
      </div>
    ...
    async handleCreatePOGroup(){
      ...
      if(this.page_function == 'CreateDebitNote'){
        ...
        if(status){
          ...
          this.$router.push({name: "debitnote", params:{data: JSON.stringify() }});
        }else{
