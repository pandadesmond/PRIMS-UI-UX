<template>
  <div v-if="$q.screen.width > 599" class="card_sides card_section">
    <div :class="page_function == 'CreateGoodReceiveNote' ? 'chip-container_2' : 'chip-container_1'">
       <div v-if="page_function == 'CreateGoodReceiveNote'" class="text-subtitle1 header_text">
         New Goods Receive Note
        </div>
        <div v-if="page_function == 'EditGoodReceiveNote'" class="text-subtitle1 header_text">
  ...
  <div v-else class="card_sides">
    ...
    <div :class="page_function == 'CreateGoodReceiveNote' ? 'chip-container_2' : 'chip-container_1'">
          <Chip v-if="page_function == 'CreateGoodReceiveNote'"  :label="$language('D0202')" :icon="'chevron_left'" text-color="grey-10" class="bold-font chip_sty back_chip_icon" @click="back()" />
          <Chip v-if="page_function == 'EditGoodReceiveNote'" :icon="'chevron_left'" :square="true" :label="'Edit Surcharge Code' +' / '+json.Itemcode" text-color="grey-10" class="bold-font chip_sty" v-on:receiveClick="handleNavigate('Itemmaster')"/>
      </div>
...
export default{
  ...
  async mounted()
  {
    ...
    if(this.page_function === 'EditGoodReceiveNote'){
  ...
  methods: {
    ...
    back(){
      this.$router.push("goodreceivenote");
    },
    ...
    async handleCreatePOGroup(){
      ...
      if(this.page_function == 'CreateGoodReceiveNote'){
        var payload = {
          "pass_json":{
            "outlet_guid": this.session_outlet_guid,
            "language": "EN",
        ...
        if(status){
          ...
          this.$router.push({name: "goodreceivenote", params:{data: JSON.stringify() }});
        }else{
      ...
      }else if(this.page_function == 'EditGoodReceiveNote'){
        var payload = {
          "pass_json":{
            "outlet_guid": this.session_outlet_guid,
            "language": "EN",
