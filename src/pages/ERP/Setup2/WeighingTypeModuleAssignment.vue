<template>
  ...
  <div class="row main_container">
      ...
      <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
          ...
          :handleActionColumn="'module_desc'"
  ...
  <q-dialog v-model="delete_weight" persistent position="top">
    <q-card style="width: 864px; max-width: 98vw;text-align:center;margin-top: 5%;border-radius: 8px">
      ...
      <q-card-actions align="right" style="padding-top:0px;padding-bottom:16px;padding-right:16px">
        ...
        <q-btn flat :label="$language('D0156')" size="12px" font_color="'white'" color="'#D81111'" class="confirm_delete_button" @click="handleDeleteWeight" /> <!-- OK -->
      </q-card-actions>
...
export default {
  ...
  methods:{
      ...
      handleShowCreateItemMaster(){
        this.$router.push({ name: "CreateWeighingTypeModuleAssignment" });
      },
      ...
      handleReceiveHandleEdit(pass_payload)
      {
          this.$router.push({
              ...
              query: { ModuleGuid: pass_payload.row.module_guid}
          });
      },
      handleReceiveHandleDelete(pass_payload)
      {
        this.get_module_guid = pass_payload.row.module_guid
        this.get_module_desc = pass_payload.row.module_guid
        // this.delete_weight = true

          // console.log('handleReceiveHandleDelete');
          // console.log(pass_payload);
      },
      async handleDeleteWeight(){
        ...
        var payload = {
          "pass_json":{
            ...
            // "module_desc": this.get_module_desc,
            "module_guid": this.get_module_guid
          }
        }

        var object_pass = {
          "dispatch": 'setup2_weighingtypemoduleassignment/trigger_delete_weight_type_module_get_weight_type_module',
          "getter": 'setup2_weighingtypemoduleassignment/get_weight_type_module_get_weight_type_module',
          "app": this,
          "payload": payload,
        };

        var dispatch_response_delete_weight_type_module_get_weight_type_module = await this.$dispatch(object_pass);

        var status = dispatch_response_delete_weight_type_module_get_weight_type_module.status

        if(status){
          this.showNotify('positive', 'Deleted Successfully')
          this.delete_weight = false
          this.deleteLoading = false
          this.table_function(this.ori_params)
        }else{
          var get_error_message = JSON.parse(dispatch_response_delete_weight_type_module_get_weight_type_module.response)
      ...
      async table_function(ori_payload){
        ...
        //     xpayload.pass_json.module_desc = this.table_column[1].filter_value;
        //   }
        ...
        if(this.table_column.length != 0){
          if(this.table_column[0].filter_value != ''){
            xpayload.pass_json.module_desc = this.table_column[0].filter_value;
          }
          if(this.table_column[1].filter_value != ''){
            xpayload.pass_json.created_at = this.table_column[1].filter_value;
          }
          xpayload.pass_json.created_by = this.table_column[2].filter_value;
          if(this.table_column[3].filter_value != ''){
            xpayload.pass_json.updated_at = this.table_column[3].filter_value;
          }
          xpayload.pass_json.updated_by = this.table_column[4].filter_value;
        }
        ...
        var status = dispatch_response_get_weight_type_module_get_weight_type_module.status
          ...
          if(status == "failed"){
            this.table_data = {
              ...
              var results = dispatch_response_get_weight_type_module_get_weight_type_module.response.detail;

              this.table_data =
              {
                  "data": {
                      "count": dispatch_response_get_weight_type_module_get_weight_type_module.response.total_row,
                      "next": null,
                      "previous": null,
                      "results": dispatch_response_get_weight_type_module_get_weight_type_module.response.detail
                  }
              };

              this.results = dispatch_response_get_weight_type_module_get_weight_type_module.response.detail;
