<template>
  <div v-if="$q.screen.width > 599" class="card_sides card_section">
    <div :class="page_function == 'CreatePOCancellation' ? 'chip-container_2' : 'chip-container_1'">
       <div v-if="page_function == 'CreatePOCancellation'" class="text-subtitle1 header_text">
         New PO Cancellation
        </div>
        <div v-if="page_function == 'EditPOCancellation'" class="text-subtitle1 header_text">
          Edit PO Cancellation
        </div>
    </div>
  ...
  <div v-else class="card_sides">
    ...
    <div :class="page_function == 'CreatePOCancellation' ? 'chip-container_2' : 'chip-container_1'">
          <Chip v-if="page_function == 'CreatePOCancellation'"  :label="$language('D0202')" :icon="'chevron_left'" text-color="grey-10" class="bold-font chip_sty back_chip_icon" @click="back()" />
          <Chip v-if="page_function == 'EditPOCancellation'" :icon="'chevron_left'" :square="true" :label="'Edit Surcharge Code' +' / '+json.Itemcode" text-color="grey-10" class="bold-font chip_sty" v-on:receiveClick="handleNavigate('Itemmaster')"/>
      </div>
...
export default{
  ...
  async mounted()
  {
    ...
    if(this.page_function === 'EditPOCancellation'){
  ...
  methods: {
    ...
    back(){
      this.$router.push("pocancellation");
    },
    ...
    async handleCreatePOGroup(){
      ...
      if(this.page_function == 'CreatePOCancellation'){
        var payload = {
          "pass_json":{
            "outlet_guid": this.session_outlet_guid,
            "language": "EN",
        ...
        if(status){
          ...
          this.$router.push({name: "pocancellation", params:{data: JSON.stringify() }});
        }else{
      ...
      }else if(this.page_function == 'EditPOCancellation'){
        var payload = {
          "pass_json":{
            "outlet_guid": this.session_outlet_guid,
            "language": "EN",
