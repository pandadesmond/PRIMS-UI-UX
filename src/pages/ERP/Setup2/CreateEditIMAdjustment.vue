<template>
  <div v-if="$q.screen.width > 599" class="card_sides card_section">
    <div :class="page_function == 'CreateAdjustment' ? 'chip-container_2' : 'chip-container_1'">
       <div v-if="page_function == 'CreateAdjustment'" class="text-subtitle1 header_text">
         New Adjustment
        </div>
  ...
  <div v-else class="card_sides">
    ...
    <div :class="page_function == 'CreateAdjustment' ? 'chip-container_2' : 'chip-container_1'">
          <Chip v-if="page_function == 'CreateAdjustment'"  :label="$language('D0202')" :icon="'chevron_left'" text-color="grey-10" class="bold-font chip_sty back_chip_icon" @click="back()" />
          <Chip v-if="page_function == 'EditAdjustment'" :icon="'chevron_left'" :square="true" :label="'Edit Surcharge Code' +' / '+json.Itemcode" text-color="grey-10" class="bold-font chip_sty" v-on:receiveClick="handleNavigate('Itemmaster')"/>
      </div>
    ...
    async handleCreatePOGroup(){
      ...
      if(this.page_function == 'CreateAdjustment'){
        ...
        if(status){
          ...
          this.$router.push({name: "adjustment", params:{data: JSON.stringify() }});
        }else{
