    ...
    methods: {
        ...
        async table_function(payload){
          ...
          var category_results = {};
            ...
            await this.$store.dispatch('setup/trigger_get_mc_division', payload).then(() => {
                division_data = JSON.parse(JSON.stringify(this.$store.getters['setup/get_mc_division']));
            });
            ...
            await this.$store.dispatch('setup/trigger_get_mc_department', payload).then(() => {
                dept_data = JSON.parse(JSON.stringify(this.$store.getters['setup/get_mc_department']));
                dept_results = dept_data.data.results;
                for(var i in dept_results)
                {
                    var obj = dept_results[i];
            ...
            await this.$store.dispatch('setup/trigger_get_mc_subdept', payload).then(() => {
                subdept_data = JSON.parse(JSON.stringify(this.$store.getters['setup/get_mc_subdept']));
                subdept_results = subdept_data.data.results;
                for(var i in subdept_results)
                {
                    var obj = subdept_results[i];
            ...
            await this.$store.dispatch('setup/trigger_get_mc_category', payload).then(() => {
                category_data = JSON.parse(JSON.stringify(this.$store.getters['setup/get_mc_category']));
                category_results = category_data.data.results;
                for(var i in category_results)
                {
                    var obj = category_results[i];
            ...
            division_results = division_data.data.results;

            for(var j in division_results)
            {
              var diobj = division_results[j];
              diobj.expand = false;
              diobj.dept = [];

              for(var k in dept_results)
              {
                var deobj = dept_results[k];
                deobj.expand = false;
                deobj.subdept = [];

                if(deobj.division_guid.division_guid == diobj.division_guid)
                {
                  diobj.dept.push(deobj)
                }

                for(var l in subdept_results)
                {
                  var sdobj = subdept_results[l];
                  sdobj.expand = false;
                  sdobj.category = [];

                  if(sdobj.department_guid.department_guid == deobj.department_guid)
                  {
                    deobj.subdept.push(sdobj)
                  }

                  for(var m in category_results)
                  {
                    var caobj = category_results[m];
                    caobj.expand = false;

                    if(caobj.subdept_guid.subdept_guid == sdobj.subdept_guid)
                    {
                      sdobj.category.push(caobj)
                    }
                  }

                  var ary = [];

                  Object.keys(sdobj.category).forEach((i)=>{
                    var object = sdobj.category[i]
                    var json_body = {
                      "action": true,
                      "category_code": object.code,
                      "category_desc": object.description,
                ...
                Object.keys(deobj.subdept).forEach((i)=>{
                  var object = deobj.subdept[i]
                  var json_body = {
                    "action": true,
                    "subdept_code": object.code,
                    "subdept_desc": object.description,
                    "created_at": object.created_at,
                    "created_by": object.created_by,
                    "updated_at": object.updated_at,
                    "updated_by": object.updated_by,
              ...
              Object.keys(diobj.dept).forEach((i)=>{
                var object = diobj.dept[i]
                var json_body = {
                  "action": true,
                  "dept_code": object.code,
                  "dept_desc": object.description,
                  "created_at": object.created_at,
                  "created_by": object.created_by,
                  "updated_at": object.updated_at,
                  "updated_by": object.updated_by,
