<template>
    ...
    >
      <template v-slot:selected>
        ...
        <template v-else>
            <div style="color: grey">Job Status</div>
        </template>
      </template>
    </q-select>
...
export default {
    ...
    props: ['value', 'options', 'label', 'id', 'name','dbComponentBehavior', 'no_label', 'loading','select'],
    methods: {
      handleChange (newVal) {
        var filter_label = this.options.filter((entry)=>{
