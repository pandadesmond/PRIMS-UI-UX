  ...
  <q-card-section v-if="json_container.title">
    <div class="row justify-between">
     <div class="col-xs-10 col-sm-10 col-md-10 col-lg-10 col-xl-10 text-h6 custom_font" style="font-size: 17px;">
      <span class="custom_title">{{json_container.title}}</span>
     </div>
    ...
    <q-icon v-if="json_container.edit" class="edit_icon" name="edit" @click="goEdit">
     <q-tooltip  anchor="top middle" self="bottom middle">
      <span class="custom_tooltip">{{json_container.edit}}</span>
     </q-tooltip>
    </q-icon>
   ...
   <div v-if="json_container.extra_title" class="text-bold" style="color: #51486c; padding-left: 31px; padding-top: 10px;">
   ...
   <q-card-section v-else :style="$q.screen.width >= 1366 ? '' : 'padding-left: 0px; padding-right: 0px;'">
    <apexchart v-if="json_container.name == 'Bar_load'" ref="graph" @updated="updatedhandler2" @animationEnd="animationEndhandler" @dataPointSelection="dataPointSelectionHandler" :height="json_container.chartOptions.chart.height" :width="json_container.chartOptions.chart.width" :options="json_container.chartOptions" :series="json_container.series"></apexchart>
    <apexchart v-else ref="graph" @updated="updatedhandler" @animationEnd="animationEndhandler" @dataPointSelection="dataPointSelectionHandler" :height="json_container.chartOptions.chart.height" :options="json_container.chartOptions" :series="json_container.series"></apexchart>
   </q-card-section>    
...
export default {
  name: 'BarChart',
  props: ['json',"forceLoading","forceLoading2", "forceLoading3", "forceLoading10", "ordering",'forceLoadings', 'disables'],
  data(){
    return{
        ...
        updatedhandler(){
         ...
         if(length <= 1){
          setTimeout(()=>{
            this.$emit('BarChartAnimationHandle');
            this.loading = false;
          },3000); // chnage here if there's problem in filter or other stuff related to updatehandler
         }
        },
        updatedhandler2(){
         if(this.json_container.chartOptions.labels === undefined){
         ...
         if(length <= 1){
          setTimeout(()=>{
            this.$emit('BarChartAnimationHandle');
            this.loading = false;
          },10000); // chnage here if there's problem in filter or other stuff related to updatehandler
         }
        },
        handleClick(newVal){
          var key = newVal.key;
    ...
    forceLoading10(newVal){
     if(newVal){
      this.loading = true;
      this.$emit('edit_forceLoading10'); // change false then return back to true
     }
    },
