            ...
            <!-- <q-td :style="[props.row.UpdateStat > 0 ? '' : '']"> -->
              <q-td :style="[props.row.UpdateStat > 0 ? 'color:red;' : '']"><!--need to add back-->
                ...
                <q-tooltip anchor="top right" v-if="props.row.UpdateStat > 0" :style="`font-size:13px;font-weight:bold;`">
                    <template v-if="props.row.LastUpdatedDate == 0">* No Longer Update</template>
                    <template v-else>
...
export default {
    ...
    props:['title', 'table_column', 'table_data', 'row_per_page', 'hide_footer', 'forceLoading','table_footer','action_button','edit_button','view_button','pass_class', 'search_button'],
    created() {
    },
    components:{
        Table,
