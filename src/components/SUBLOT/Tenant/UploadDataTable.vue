       ...
       <template v-slot:top-right v-if="top_button" >
        ...
        <q-tr @click="handleRowClick(props)" :props="props"  :style="[`cursor: pointer;`,props.row.active_row ? `background-color: #eeeeee;` : ``]">
          ...
          <q-td v-if="col.name == 'action'">
            ...
            <q-btn icon="edit_note"  dense :outline="button_no_outline" ripple class="q-mr-sm button" v-on:click="action(props.row)"
            v-if="action_button">
                <q-tooltip>
                    Edit
                </q-tooltip>
...
export default {
    ...
    computed: {
      dbComponentBehavior() {
        return this.$store.getters['dbComponentBehavior/byLanguage']('')
      },
      // pagesNumber() {
