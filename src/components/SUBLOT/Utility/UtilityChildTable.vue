            ...
            <q-tr >
                ...
                <template v-for="column_details in props.cols" :key="column_details">
                  <q-td v-if="column_details.name == 'checkbox'" :style="['text-align:'+column_details.align+';', column_details.style]">
                    ...
                    <q-td v-else-if="column_details.name == 'PaymentAmt'" :style="['text-align:'+alignedValue(props.row[column_details.name])+';', column_details.style]">
                            <!-- {{props.row[column_details.name]}} -->
                            {{formatAmount(props.row[column_details.name],'$')}}
                    </q-td>


                    <q-td v-else-if="column_details.name == 'LocalPaymentAmt'" :style="['text-align:'+alignedValue(props.row[column_details.name])+';', column_details.style]">
                            <!-- {{props.row[column_details.name]}} -->
                            {{formatAmount(props.row[column_details.name],'$')}}
                    </q-td>


                    <q-td v-else-if="column_details.name == 'KnockOffAmt'" :style="['text-align:'+alignedValue(props.row[column_details.name])+';', column_details.style]">
                            <!-- {{props.row[column_details.name]}} -->
                            {{formatAmount(props.row[column_details.name],'$')}}
                    </q-td>

                    <q-td v-else-if="column_details.name == 'LocalUnappliedAmount'" :style="['text-align:'+alignedValue(props.row[column_details.name])+';', column_details.style]">
                            <!-- {{props.row[column_details.name]}} -->
                            {{formatAmount(props.row[column_details.name],'$')}}
                    </q-td>

                    <q-td v-else-if="column_details.name == 'RefundAmt'" :style="['text-align:'+alignedValue(props.row[column_details.name])+';', column_details.style]">
                            <!-- {{props.row[column_details.name]}} -->
                            {{formatAmount(props.row[column_details.name],'$')}}
                    </q-td>

                    <q-td v-else-if="column_details.name == 'isinvoice'" :style="['text-align:'+alignedValue(props.row[column_details.name])+';', column_details.style]">
                            <!-- {{props.row[column_details.name]}} -->
                            <!-- {{formatAmount(props.row[column_details.name],'$')}} -->

<!-- 
                            <q-checkbox v-if="props.row[column_details.name] == '1'" v-model="props.row[column_details.name]"
                            :true-value= "1"
                            :false-value = "0"
                             /> -->

                            <template v-if="formatAmount(props.row[column_details.name]) == '1'">
                                <q-checkbox v-model="props.row[column_details.name]"
                                :true-value= "1"
                                :false-value = "0"
                                disable
                                />
                            ...
                            <!-- <template v-if="formatAmount(props.row[column_details.name]) == '0'">
                                <q-checkbox v-model="checked_false" disable/>
                            </template> -->


                            <!-- <span v-else>checkbox</span> -->
                             <!-- <q-checkbox v-if="props.row[column_details.name] == '0'"/> -->
...
export default {
  ...
  props: ['table_column', 'table_data', 'delete_button', 'view_button', 'edit_button', 'print_button','xselectAll', 'table_title', 'hide_search'],
  methods:{
