          ...
          <!-- <template v-for="details in rows" :key="details"> -->
              ...
              <q-tr :props="props">
                  <template v-for="(value,header,index) in props.row" :key="index">
                      ...
                      <!-- <q-td v-if="header == 'dateto'" :class="props.cols[index].classes" :style="['cursor: context-menu;text-align: left;','text-align:'+props.cols[index].align+';', 'background-color: #00000008;']">
                      ...
                      <q-td v-if="header == 'total'"  :class="props.cols[index].classes" :style="['cursor: context-menu;text-align: left;','text-align:'+props.cols[index].align+';', 'background-color: #00000008;']">
                        ...
                        <span>
                          ...
                          <q-tooltip anchor="top middle" self="bottom middle">
                            ...
                            <b>Power Factor:</b> {{props.row.power_factor}}<br>
                            ...
                            <template v-else>
                              ...
                              <template v-if="props.row.flat_rate.length > 0">

                                <template v-for="(level_details, xindex) in props.row.flat_rate" :key="xindex">

                                  <span v-if="props.row.flat_rate.length == (xindex+1)">
                      ...
                      <q-td v-if="header == 'icpt'"  :class="props.cols[index].classes" :style="['cursor: context-menu;text-align: left;','text-align:'+props.cols[index].align+';', 'background-color: #00000008;']">
                          ...
                          <!-- <b>({{formatAmount(props.row[header].rate,'$',2)}}%)</b>
                      ...
                      <q-td v-if="header == 'flat_rate_hide'" :class="props.cols[index].classes" :style="['cursor: context-menu;text-align: left;','text-align:'+props.cols[index].align+';', 'background-color: #00000008;']">
                      ...
                      <q-td v-if="header != 'flat_rate' && header != 'power_factor' && header != 'agr_guid' && header != 'pure_new_record' && header != 'previous_month_no_close_details' && header != 'utility_rate_guid_for_save_user' && header != 'new_data' && header != 'previous_month_no_close' && header != 'utility_rate_transaction_guid' && header != 'assigned' && header != 'got_opening' && header != 'lot' && header != 'tenant' && header != 'last' && header != 'usage' && header != 'flat_rate' && header != 'kwtbb' && header != 'icpt' && header != 'total' && header != 'grand_total' && header != 'utility_rate_guid' && header != 'sublot_guid' && header != 'estimated'" :style="['cursor: pointer;','text-align:'+props.cols[index].align+';']" @click="handleTdClick(props.row[header], header, props.row)">
                          <!-- {{props}} -->
                          <!-- {{props.cols[index]}} -->
                          <!-- {{header}} -->
                          <!-- {{ props.row.agr_guid }} -->

                          <!-- <b v-if="props.row.utility_rate_guid == '' && props.row.assigned && props.row.previous_month_no_close" style="margin-right: 5px;">Please ensure existing records closed ({{ props.row.previous_month_no_close_details[0].year_month }})</b> -->
                          <b v-if="props.row.assigned && props.row.previous_month_no_close" style="margin-right: 5px;">
                            <!-- Please ensure existing records closed -->
                            Please ensure records is invoiced. 
                             ({{ props.row.previous_month_no_close_details[0].year_month }})</b>


                          <!-- <b v-else-if="props.row.utility_rate_guid == '' && props.row.assigned" style="margin-right: 5px;">NEW</b> -->

                          <b v-if="props.row.pure_new_record" style="margin-right: 5px;">(NEW)</b>
                          
                          <template v-if="props.row.assigned && !props.row.previous_month_no_close">
                            {{formatAmount(props.row[header],'$',2)}}
                            <q-popup-edit style="padding: 0px;"  @show="focusInput" v-model="props.row[header]" v-slot="scope"  @save="handleSave($event,props,index)">
                                ...
                                <q-card>
                                    <q-card-section style="padding-bottom: 0px;">
                                        ...
                                        <div style="display: flex; align-items: center;">
                                          ...
                                          <span style="display: flex; align-items: center; width: 100%; text-align: right;">
                                            ...
                                            <q-checkbox v-if="input_field_type == 'text' || input_field_type == 'number'" v-model="estimation" />
                                    ...
                                    <q-card-section style="padding-bottom: 5px;">
                                        <q-input v-if="input_field_type == 'text' || input_field_type == 'number'" :type="input_field_type" :class="input_field_type == 'number' && 'text_align_right'" ref="inputRef" v-model="newValue"  @input="scope.value = formattedValue" @keyup.enter="handlePopupSave(value,header,index,props, scope)"/>


                                        <Datepicker v-if="input_field_type == 'date'"
                                          :readonly="false"
                                          :no_label="true"
                                          :autoclose="autoclose"
                                          :min_year="min_year"
                                          :max_year="max_year"
                                          :daterange="newValue"
                                          v-on:receiveChange="handleChangeDate"
                                          :componentBehavior="dbComponentBehavior.required_date"
                                          :optionsFn="handleChangeInvoiceDate"
                                        />
                          ...
                          <template v-else>
                            ...
                            <!-- <b v-if="!props.row.previous_month_no_close">Not assign to agreement</b> -->
                            <b v-if="!props.row.previous_month_no_close">Not assigned</b>
                          </template>
...
const optionsyear_pass = [year-2,year-1,year];
const month = ["January","February","March","April","May","June","July","August","September","October","November","December"];
let monthname = month[currentdate.getMonth()];
var month_filter = ('0' + ('0' + (currentdate.getMonth()+1)).slice(-2)).slice(-2);
...
export default {
  ...
  async mounted()
  {
    ...
    await this.$store.dispatch('setup/trigger_company', payload).then(() => {
      var data = JSON.parse(JSON.stringify(this.$store.getters['setup/get_company']));

      if(data.status = "success")
      {
        if(data.data.results.length > 0)
        {
          this.company_details = data.data.results[0];
        }
        else
        {
  ...
  methods:{
    handleChangeInvoiceDate(date)
    {
      ...
      // console.log(this.company_details.invoice_date_period);
      if(this.company_details.invoice_date_period)
      {
        var end_period_code = this.company_details.invoice_date_period.replaceAll("-", "/");
        return date >= end_period_code;
      }
      else
      {
        return date
...
// console.log('until here, need to defined back all using rate one using rate. For example, flat rate using dollar sign for data formatting. Maybe can correct previous data type first with previous method see work or no formatting');
    ...
    handlePopupSave(value,header,column_index,payload, event)
    {
        ...
        {
          this.showNotify("negative","Value cannot be empty");
          return;
        }
